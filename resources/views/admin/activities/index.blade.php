            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    <thead class="bg-light">
                        <tr>
                            <th class="py-3 px-4 text-uppercase text-muted fw-bold" style="font-size: 0.85rem; width: 8%;">
                                Ikona</th>
                            <th class="py-3 px-4 text-uppercase text-muted fw-bold" style="font-size: 0.85rem; width: 35%;">
                                Název a popisek</th>
                            <th class="py-3 px-4 text-uppercase text-muted fw-bold" style="font-size: 0.85rem; width: 20%;">
                                Ceny (Hodina / Den)</th>
                            <th class="py-3 px-4 text-uppercase text-muted fw-bold" style="font-size: 0.85rem; width: 22%;">
                                Štítek rozvrhu</th>
                            <th class="py-3 px-4 text-uppercase text-muted fw-bold text-end"
                                style="font-size: 0.85rem; width: 15%;">Akce</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($activities as $activity)
                            <tr>
                                ...
                                {{-- Ceny (Hodinová a Paušální) --}}
                                <td class="px-4 py-3">
                                    @if ($activity->price_per_hour > 0 || $activity->price_per_day > 0)
                                        <div class="fw-bold text-dark mb-1">
                                            {{ number_format($activity->price_per_hour, 0, ',', ' ') }} Kč <span
                                                class="text-muted fw-normal small">/ hod</span>
                                        </div>
                                        <div class="text-muted small">
                                            {{ number_format($activity->price_per_day, 0, ',', ' ') }} Kč <span
                                                class="fw-normal">/ den</span>
                                        </div>
                                    @else
                                        <span
                                            class="badge bg-success bg-opacity-10 text-success border border-success rounded-pill px-3 py-1 fw-semibold">
                                            Zdarma / Dobrovolně
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td class="px-4 py-3 text-end">
                                    <div class="d-flex justify-content-end gap-2">
                                        <a href="{{ route('admin.activities.edit', $activity->id) }}"
                                            class="btn btn-sm btn-outline-warning text-dark" title="Upravit aktivitu">
                            ...
                            <tr>
                                <td colspan="5" class="text-center py-5">
                                    <div class="text-muted">
                                        ...
                                        <p class="small mb-0">Kliknutím na tlačítko Nová aktivita založíte první položku pro
                                            rezervace.</p>
                                    </div>
                                </td>
                            </tr>
